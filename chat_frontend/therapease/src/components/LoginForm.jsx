...
const LoginForm = () => {
  ...
  return (
    <form
      ...
      className={`w-[332px] h-[413px] flex  flex-col  `}
    >
      <div className="">
        <h2 className="text-3xl font-bold mb-3 md:bg-graylight">Login</h2>
        <p className="text-gray leading-20 py-2 max-w-[255px]">
          Welcome back! Please login to your account.
        </p>
      </div>
      <div className="mb-4 grid ">
        <label htmlFor="email" className="block mb-1 py-2 text-gray">
          Email
        </label>
        <input
          type="email"
      ...
      <div className="mb-4">
        ...
        />
        {error && password.trim() === "" && <p className="text-red"></p>}
        {error && !validatePassword(password) && (
          <p className="text-red">
            Password must be at least 6 characters long, include atleast 1
            Uppercase letter, 1 lowercase letter, 1 numeric value and 1 special
            character
          </p>
        )}
      </div>
      <div className="flex items-center justify-between mb-4 py-2">
        <label htmlFor="remember" className="flex items-center   text-gray">
          <input
            type="checkbox"
            id="remember"
            className={`mr-2 mt-1 bg-red ${
              error ? "bg-red" : "bg-white"
            } checked `}
          />
          Remember me
        </label>
...
//           if (userEmail==null){
//             dispatch(addDetail({ detail: { email:email} }));
//           }
//           navigate("/")
//         }
//         else{
